    ...
    def setGame(self, name):
        ...
        def setVideo(source):
            ...
            if self.video is None:
                self.video = tkVideo(master=self, width=self.videoW, height=self.videoH, autostart=False)
                self.video.place(x = self.WIDTH - self.videoW - self.PAD_X, 
                                 y = self.HEIGHT - self.videoH - self.PAD_Y)
                
            if len(files) > 0:
                self.video.setSource(files[0], autostart=True)
            else:
                self.video.stop()
        ...
        self.itemconfig(self.text[1], text="")
        threading.Thread(target=lambda: setBg(self.APP_PATH + "images/" + name + ".*")).start()
        threading.Thread(target=lambda: setText(self.APP_PATH + "texts/" + name + ".txt")).start()
        threading.Thread(target=lambda: setVideo(self.APP_PATH + "videos/" + name + ".*")).start()
